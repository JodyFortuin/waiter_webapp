module.exports = function main(pool){
   ...
   */
   ...
   const workflow = async (userName, daysIds) => {
      ...
      const userId = await pool.query ('select id from waiters where waiter=$1', [userName]) // return id
console.log(userId)
      if(userId) {
...
*/
   ...
   async function addUser(userName){
    const INSERT_QUERY = await pool.query('insert into waiters(waiter) values($1)', [userName]);
 }
   ...
   const getUserId = async (userName) => {
     // select where name = ? => [] 
     const sql = await pool.query('select id from waiters where waiter =$1',[userName]) // edit this
    try {
